...
function LessonDashboard({ onLessonSelected, onReset }) {
  ...
  // Function to handle clicking a category header - auto-activate the single lesson
  const handleToggle = (index) => {
    const category = Categories[index];
    if (category.lessons.length > 0) {
      // First reset to clean state, then activate the lesson
      if (onReset) {
        onReset();
      }
      
      // Small delay to ensure reset completes before activation
      setTimeout(() => {
        const lesson = category.lessons[0];
        handleLessonClick(lesson, category);
      }, 100);
    }
  };
      ...
      <div className="flex-1 overflow-auto space-y-2 pr-2" style={{ WebkitOverflowScrolling: 'touch' }}>
        {Categories.map((category, index) => (
          <div key={index} className="border-b border-gray-200 last:border-b-0">
            {/* Category Header Button - One Click Activation */}
            <button
              onClick={() => handleToggle(index)}
              className="flex items-center justify-between w-full p-3 text-left rounded-lg hover:bg-blue-100 transition-colors"
              title={`Click to start ${category.title}`}
            >
              <div className="flex-1">
                <div className="text-sm md:font-semibold text-gray-800">{category.title}</div>
                {category.lessons.length > 0 && (
                  <div className="text-xs text-gray-500 mt-1">{category.lessons[0].description}</div>
                )}
              </div>
