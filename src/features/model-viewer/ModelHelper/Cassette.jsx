...
function Cassette({
  ...
  const { scene } = useGLTF('/Model/cassette.glb');
    ...
    if (mainObj) {
      ...
      const captureBaseline = () => {
        ...
        if (baselineZ !== null && originalZRef.current === null) {
          ...
          console.log('Cassette: Using provided baseline Z:', originalZRef.current);
        } else if (originalZRef.current === null) {
          // First time - capture current Z as baseline
          originalPositionRef.current = mainObj.position.clone();
          originalZRef.current = mainObj.position.z;
          console.log('Cassette: Captured baseline Z:', originalZRef.current);
      ...
      console.warn('Cassette: object named "main" not found in GLB scene.');
    }
  ...
  useEffect(() => {
    ...
    // Ensure parents update their matrices
    let parent = mainObj.parent;
    while (parent) {
      parent.updateMatrixWorld(true);
      parent = parent.parent;
    ...
    console.log('Cassette: Updated position - Original Z:', originalZ, 'Height Offset:', heightOffset, 'Final Z:', mainObj.position.z);
...
useGLTF.preload('/Model/cassette.glb');
